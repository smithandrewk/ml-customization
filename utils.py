...
def get_verified_and_not_deleted_sessions(project_name, labeling):
    ...
    cursor = conn.cursor()
    # Execute a query
    query = f"SELECT session_name,bouts FROM sessions WHERE project_id='{get_project_id(project_name)}' AND verified=1 AND (keep IS NULL OR keep != 0)"  # Replace with your actual table name
    cursor.execute(query)
