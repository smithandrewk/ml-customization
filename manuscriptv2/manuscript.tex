...
\newcommand{\nParticipants}{13}
\newcommand{\baseFoneMean}{0.709}
\newcommand{\baseFoneStd}{0.213}
\newcommand{\baseFoneMax}{0.964}
\newcommand{\baseFoneMin}{0.432}
\newcommand{\finetuneFoneMean}{0.836}
\newcommand{\finetuneFoneStd}{0.126}
\newcommand{\finetuneFoneMax}{0.980}
\newcommand{\finetuneFoneMin}{0.621}
\newcommand{\absoluteImprovementMean}{0.127}
\newcommand{\absoluteImprovementStd}{0.097}
\newcommand{\absoluteImprovementMax}{0.268}
\newcommand{\absoluteImprovementMin}{0.003}
\newcommand{\relativeImprovementMean}{23.7}
\newcommand{\relativeImprovementStd}{21.9}
\newcommand{\relativeImprovementMax}{61.9}
\newcommand{\relativeImprovementMin}{0.3}
\newcommand{\relativeRoomForImprovementMean}{41.3}
\newcommand{\relativeRoomForImprovementStd}{13.6}
\newcommand{\relativeRoomForImprovementMax}{57.2}
\newcommand{\relativeRoomForImprovementMin}{7.7}
\newcommand{\precisionMean}{0.861}
\newcommand{\precisionStd}{0.110}
\newcommand{\precisionImprovementMean}{0.097}
\newcommand{\recallMean}{0.828}
\newcommand{\recallStd}{0.131}
\newcommand{\recallImprovementMean}{0.090}
% 1% data regime metrics
\newcommand{\onePctFinetuneMedian}{0.730}
\newcommand{\onePctTargetOnlyMedian}{0.494}
\newcommand{\onePctAbsoluteImprovement}{0.236}
\newcommand{\onePctRetentionPct}{83.0}
% Dataset statistics - \textcolor{red}{TODO: Verify these calculated values from eval\_demographics.ipynb}
...
\newcommand{\numParams}{50,753}  % \textcolor{red}{TODO: Compute exact parameter count for base\_channels=64}
% Title
...
We employed leave-one-participant-out (LOPO) cross-validation across \nParticipants~participants. Each participant's data were split into train (60\%), validation (20\%), and test (20\%) sets at the session level as described in Section 2.1. For each LOPO fold, one participant served as the target for personalization and evaluation, while the remaining 12 participants formed the base population.
...
\hline
0 & 0.84 & 0.93 & +0.09 & 10.8 & 0.94 & 0.93 & +0.15 & +0.00 \\
1 & 0.67 & 0.83 & +0.16 & 23.4 & 0.87 & 0.80 & +0.05 & +0.18 \\
2 & 0.96 & 0.97 & +0.00 & 0.3 & 0.98 & 0.96 & +0.01 & -0.00 \\
3 & 0.43 & 0.70 & +0.27 & 61.9 & 0.75 & 0.71 & +0.05 & +0.19 \\
4 & 0.96 & 0.97 & +0.01 & 1.5 & 0.97 & 0.97 & +0.02 & +0.01 \\
5 & 0.88 & 0.93 & +0.05 & 5.7 & 0.93 & 0.93 & +0.06 & +0.04 \\
6 & 0.44 & 0.68 & +0.23 & 52.1 & 0.72 & 0.68 & +0.00 & +0.17 \\
7 & 0.49 & 0.72 & +0.22 & 45.7 & 0.77 & 0.68 & +0.28 & +0.19 \\
8 & 0.68 & 0.85 & +0.18 & 26.0 & 0.90 & 0.83 & +0.27 & +0.03 \\
9 & 0.89 & 0.93 & +0.04 & 4.6 & 0.95 & 0.91 & +0.07 & +0.02 \\
10 & 0.52 & 0.77 & +0.25 & 49.1 & 0.81 & 0.76 & +0.11 & +0.23 \\
11 & 0.96 & 0.98 & +0.02 & 2.1 & 0.96 & 1.00 & +0.03 & +0.00 \\
12 & 0.50 & 0.62 & +0.12 & 24.3 & 0.64 & 0.61 & +0.14 & +0.11 \\
\textbf{Mean} & \textbf{0.709} & \textbf{0.836} & \textbf{0.127} & \textbf{23.7} & \textbf{0.861} & \textbf{0.828} & \textbf{0.097} & \textbf{0.090} \\
\textbf{Std} & \textbf{0.213} & \textbf{0.126} & \textbf{0.097} & \textbf{21.9} & \textbf{0.110} & \textbf{0.131} & \textbf{0.092} & \textbf{0.090} \\
\hline
\end{tabular}
\end{table}
...
The critical advantage of transfer learning emerges when individual data is limited. We systematically evaluated both approaches across data-scarce regimes by training models with 1\%, 5\%, 12.5\%, 25\%, 50\%, and 100\% of each participant's data (Figure \ref{fig:low_data_comparison}). The results reveal a dramatic divergence in data efficiency between methods.

At the most extreme data scarcity (1\% of target data, approximately 1.2 hours per participant), the superiority of transfer learning is most pronounced. Target-only models trained from scratch collapsed to median F1 of \onePctTargetOnlyMedian, barely exceeding chance performance and exhibiting high variance across participants. In stark contrast, fine-tuned models maintained robust performance with median F1 of \onePctFinetuneMedian, demonstrating an absolute improvement of \onePctAbsoluteImprovement~F1 points over target-only training. This improvement at 1\% data is nearly twice the magnitude of the mean improvement from base to fine-tuned models at full data (\absoluteImprovementMean), underscoring the critical value of population-pretrained representations when individual labels are extremely scarce. Remarkably, fine-tuning with just 1\% of target data retains \onePctRetentionPct\% of the performance achieved with 100\% of target data, while target-only training at 1\% achieves only 56.6\% of its full-data performance—a 26.4 percentage point difference in retention. At this extreme, fine-tuned models with 1\% data approach the performance of base population models (median \onePctFinetuneMedian~vs. mean \baseFoneMean), demonstrating that minimal individual data is sufficient to match or exceed generic population models.

At 5\% of target data (approximately 6 hours per participant), target-only models improved to median F1 $\approx$ 0.71 but still exhibited substantial variance, indicating unreliable personalization. In contrast, fine-tuned models maintained robust performance (median F1 $\approx$ 0.80), approaching their full-data performance with just a fraction of individual labels. This represents a performance gap of approximately 0.09 F1 points at 5\% data.

The data efficiency advantage persisted across all tested fractions. At 12.5\% data (~16 hours), fine-tuned models (F1 $\approx$ 0.83) substantially outperformed target-only models (F1 $\approx$ 0.76). The two approaches converged only at 100\% data, where both achieved similar performance. Critically, fine-tuned models trained on 5\% of target data matched or exceeded the performance of target-only models trained on 100\% of target data, representing approximately 20-fold improvement in data efficiency.

These findings have profound implications for practical deployment. Collecting two weeks of labeled data (approximately 127 hours of wear time) represents a substantial user burden that limits real-world feasibility. Transfer learning dramatically reduces this requirement: with just 1\% of this data (1.2 hours), fine-tuning achieves \onePctRetentionPct\% performance retention (median F1 \onePctFinetuneMedian), while target-only training collapses to barely functional performance (median F1 \onePctTargetOnlyMedian). At 5\% data (~6 hours), fine-tuning performance approaches full-data levels. This reduction from two weeks to one hour of data collection transforms personalized smoking detection from an impractical research protocol into a deployable intervention system. This data-efficient personalization strategy generalizes beyond smoking detection to any behavioral sensing application where individual variability is high but per-user data collection is costly.
...
We demonstrate that transfer learning resolves the fundamental tension between personalization accuracy and data collection burden in wearable behavioral sensing. By fine-tuning population-pretrained models with minimal individual data, we achieve robust personalized smoking detection with as little as 1.2 hours (1\% of target data) of user data, where fine-tuning maintains median F1 of \onePctFinetuneMedian~while target-only training collapses to \onePctTargetOnlyMedian. This approach delivers mean F1 scores of \finetuneFoneMean~across \nParticipants~participants, substantially outperforming both population-only models (\baseFoneMean) and data-matched individual models in low-data regimes.
...
Most remarkably, our results demonstrate that even at the extreme of 1\% individual data (approximately 1.2 hours of wear time), fine-tuning achieves median F1 of \onePctFinetuneMedian—representing \onePctRetentionPct\% of the performance obtained with full individual data collection. This represents a practical breakthrough: users can achieve highly functional personalized detection after wearing the device for barely more than one hour, rather than two weeks. The absolute improvement of \onePctAbsoluteImprovement~F1 points over target-only training at this extreme data scarcity underscores the critical value of population pretraining. For rapid deployment scenarios, pilot studies, or populations where sustained data collection is challenging, the 1\% regime provides a viable path to personalization that was previously infeasible. At 5\% data (~6 hours), performance increases further, providing deployment flexibility based on the accuracy-burden tradeoff appropriate for each use case.
...
The optimal amount of individual data required for personalization likely varies across users. Some individuals may achieve strong performance with less than 6 hours of data, while others may benefit from additional examples. Adaptive data collection strategies that dynamically determine when sufficient personalization has been achieved could further reduce burden.
